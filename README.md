...
コードを書かずにブラウザ上で動画を編集できる GUI エディタです。
...
エディタで作成・編集した内容をローカルに保存・復元できます。

- **保存：** 上部の「Save」ボタンまたは `Ctrl+S` / `Cmd+S`
- **自動保存（AutoSave）：** 設定をオンにすると編集のたびに自動保存
- **プロジェクトファイル：** `.json` 形式でエクスポート/インポート可能
- **保存先：** ブラウザの localStorage + サーバーの `editor/dist/data/slides.json`

---

### テンプレート

あらかじめ用意されたテンプレートから動画スタイルを選択できます。
...
- ターゲット・トーン・CTA などを独自に設定して保存可能
- localStorage に最大 10 件保存
- エクスポート（`.json`）/インポートで共有可能
...
- **ファイルアップロード：** エディタの「BGM」パネルから mp3/wav をアップロード
- **音量調整：** スライダーで 0〜100% を指定
- **プレビュー：** ブラウザ上でリアルタイムに確認可能
...
- スライド切り替え時に再生する SE を選択
- SE の音量を個別に調整可能（デフォルト 80%）
- 対応形式：mp3 / wav
...
| キャプション選択移動 | `[` / `]` キー |
| 選択キャプション削除 | `Delete` キー |

履歴は最大 50 件まで保持されます。

---

*BeMyStyle Reel — Remotion Project v1.0.0*
