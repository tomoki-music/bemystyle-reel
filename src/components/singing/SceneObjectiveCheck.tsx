...
const VALUES = [0.72, 0.68, 0.80, 0.55, 0.65, 0.48];
...
const VocalRadar: React.FC<{ progress: number; size: number }> = ({ progress, size }) => {
  ...
  return (
    <svg width={size} height={size} style={{ filter: "drop-shadow(0 0 24px rgba(192,132,252,0.35))", overflow: "visible" }}>
      {grids.map((g, gi) => (
        <polygon key={gi}
      ...
      <polygon points={polygonPoints} fill="rgba(123,47,190,0.3)" stroke={PURPLE} strokeWidth={2.5} strokeLinejoin="round" />
      {points.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r={i === 5 ? 7 : 5}
      ...
      {angles.map((a, i) => {
        ...
        return (
          <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle"
            ...
            fontSize={isWeak ? 21 : 19} fontWeight={isWeak ? 700 : 500} fontFamily={FONT}
          >
            {AXES[i]}
          </text>
        );
...
const ScorePill: React.FC<{ label: string; score: number; frame: number; delay: number; fps: number }> = ({ label, score, frame, delay, fps }) => {
  ...
  return (
    <div style={{ opacity: op, display: "flex", alignItems: "center", gap: 14, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(192,132,252,0.3)", borderRadius: 40, padding: "10px 28px" }}>
      <span style={{ fontSize: 28, fontWeight: 500, color: "rgba(240,230,255,0.7)", letterSpacing: "0.08em", fontFamily: FONT }}>{label}</span>
      <span style={{ fontSize: 40, fontWeight: 900, color: PURPLE, letterSpacing: "0.04em", fontFamily: FONT, textShadow: `0 0 16px ${PURPLE}` }}>{displayScore}</span>
    </div>
  );
};
...
export const SceneObjectiveCheck: React.FC = () => {
  ...
  return (
    <ShortsLayout
      ...
      background={<SingingBackground variant="objective" />}
      brand="BeMyStyle"
      showBrand
    ...
    >
      {/* Teaser */}
      <div style={{ transform: `translateY(${teaserY}px)`, opacity: teaserOpacity, textAlign: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 52, fontWeight: 600, color: "rgba(240,230,255,0.85)", letterSpacing: "0.06em", fontFamily: FONT, textShadow: "0 2px 12px rgba(0,0,0,0.6)" }}>
          でも一番大事なのは…
        </div>
      </div>

      {/* Main headline */}
      <div style={{ transform: `translateY(${mainY}px)`, opacity: mainOpacity, textAlign: "center", marginBottom: 32 }}>
        <div style={{ fontSize: 72, fontWeight: 900, lineHeight: 1.3, letterSpacing: "0.03em", color: WHITE, fontFamily: FONT, textShadow: "0 0 40px rgba(123,47,190,0.8), 0 3px 14px rgba(0,0,0,0.8)" }}>
          自分の声を<br />
          <span style={{ color: GOLD }}>客観的に</span>知ること
        </div>
      </div>
      ...
      {/* Sub text */}
      <div style={{ transform: `translateY(${subY}px)`, opacity: subOpacity, textAlign: "center", marginTop: 32, padding: "18px 28px", background: "rgba(255,255,255,0.05)", borderRadius: 18, border: "1px solid rgba(192,132,252,0.25)" }}>
        <div style={{ fontSize: 38, fontWeight: 400, color: "rgba(240,230,255,0.85)", letterSpacing: "0.06em", fontFamily: FONT, lineHeight: 1.6, textShadow: "0 2px 8px rgba(0,0,0,0.5)" }}>
          今の課題が分かると、<br />
