...
const ConceptCard: React.FC<ConceptCardProps> = ({ icon, label, sub, color, frame, delay, fps }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 52, fontWeight: 900, color: WHITE, letterSpacing: "0.06em", fontFamily: FONT, textShadow: `0 0 20px ${color}` }}>
        {label}
      </div>
      <div style={{ fontSize: 30, fontWeight: 400, color: `${color}CC`, letterSpacing: "0.08em", fontFamily: FONT, textAlign: "center", lineHeight: 1.4 }}>
        {sub}
      </div>
    </div>
  );
...
export const SceneConclusion: React.FC = () => {
  ...
  return (
    <ShortsLayout
      ...
      contentStyle={{ padding: "0 60px" }}
    >
      {/* Headline */}
      <div style={{ transform: `translateY(${headY}px)`, opacity: headOpacity, textAlign: "center", marginBottom: 8 }}>
        <div style={{ fontSize: 80, fontWeight: 900, lineHeight: 1.25, letterSpacing: "0.02em", color: WHITE, fontFamily: FONT, textShadow: "0 0 40px rgba(123,47,190,0.9), 0 0 80px rgba(123,47,190,0.4), 0 3px 12px rgba(0,0,0,0.8)" }}>
          高音は
          <br />
          <span style={{ color: PURPLE }}>"喉の力"</span>
          じゃない
        </div>
      </div>

      {/* Sub: 3要素 */}
      <div style={{ transform: `translateY(${subY}px)`, opacity: subOpacity, textAlign: "center", marginBottom: 52 }}>
        <div style={{ fontSize: 52, fontWeight: 700, letterSpacing: "0.08em", color: GOLD, fontFamily: FONT, textShadow: "0 0 24px rgba(240,191,106,0.6), 0 2px 8px rgba(0,0,0,0.6)" }}>
          開く × 閉じる × 張る
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "row", gap: 20, width: "100%" }}>
        {CARDS.map((c) => (
          <ConceptCard key={c.label} icon={c.icon} label={c.label} sub={c.sub} color={c.color} frame={frame} delay={c.delay} fps={fps} />
        ))}
      </div>

      {/* Balance label */}
      <div style={{ opacity: balOpacity, marginTop: 36, fontSize: 38, fontWeight: 400, color: "rgba(240,230,255,0.75)", letterSpacing: "0.1em", fontFamily: FONT, textAlign: "center", textShadow: "0 2px 8px rgba(0,0,0,0.5)" }}>
        この3つのバランスが大事
      </div>
    </ShortsLayout>
  );
