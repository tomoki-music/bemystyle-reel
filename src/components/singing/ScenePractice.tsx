        ...
        {rings.map((offset, i) => {
          ...
          return (
            <ellipse
              ...
              rx={r} ry={r * 0.38}
              stroke={PURPLE}
              strokeWidth={Math.max(0.5, 2.5 - phase * 2)}
              fill="none"
              opacity={Math.max(0, alpha)}
            />
          );
        })}
        {/* Center dot – vocal source */}
...
const AscendingWave: React.FC<{ frame: number; opacity: number }> = ({ frame, opacity }) => {
  ...
  for (let i = 0; i <= 80; i++) {
    const t = i / 80;
    const x = t * totalX;
    // Wave that gradually ascends (baseline drops as t increases)
  ...
  return (
    <div style={{ opacity }}>
      <svg width={900} height={160} viewBox="0 0 900 160" overflow="visible">
        ...
        />
        {/* Gold highlight on last portion */}
        <path
          d={path}
          fill="none"
          stroke={GOLD}
    ...
    <AbsoluteFill style={{ opacity: sceneOpacity }}>
      ...
      >
        ...
        >
          ...
          >
            この感覚を
            <span style={{ color: PURPLE, fontWeight: 700 }}>まとめて</span>
            掴みやすい
          </div>
        </div>
      </AbsoluteFill>
