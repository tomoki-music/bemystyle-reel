...
interface BarRowProps {
  label: string;
  unit: string;
  barPct: number;   // 0-100, visual bar fill
  barColor: string;
  frame: number;
  delay: number;
  fps: number;
  disabled?: boolean; // 喉0割 → dimmed
}
...
const BarRow: React.FC<BarRowProps> = ({ label, unit, barPct, barColor, frame, delay, fps, disabled }) => {
  ...
  const rowOpacity = interpolate(f, [0, 14], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const barWidth = interpolate(f, [8, 40], [0, barPct], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  return (
    <div
      style={{
        transform: `translateY(${rowY}px)`,
        opacity: rowOpacity * (disabled ? 0.45 : 1),
        marginBottom: 36,
        width: "100%",
      }}
    >
      ...
      >
        ...
        <span
          style={{
            fontSize: 44,
            fontWeight: 900,
            color: disabled ? "rgba(200,200,220,0.5)" : barColor,
            letterSpacing: "0.04em",
            fontFamily: FONT,
            textShadow: disabled ? "none" : `0 0 20px ${barColor}`,
          }}
        >
          {unit}
        </span>
      ...
      >
        <div
          style={{
            ...
            borderRadius: 8,
            boxShadow: disabled ? "none" : `0 0 12px ${barColor}80`,
          }}
        />
      </div>
    </div>
...
export const SceneAdvice: React.FC = () => {
  ...
  // Body figure SVG animation (silhouette with weight center)
      ...
      >
        ...
        >
          <div
            style={{
              ...
              fontFamily: FONT,
              textShadow: `
                0 0 50px rgba(123,47,190,0.8),
                0 3px 16px rgba(0,0,0,0.8)
              `,
            }}
          >
            まずは
            <span style={{ color: GOLD }}>脱力</span>
          </div>
        </div>
        ...
        >
          <svg width={120} height={180} viewBox="0 0 120 180" fill="none">
            ...
            {/* Torso */}
            <path d="M38 42 Q38 90 42 100 L60 110 L78 100 Q82 90 82 42 Z"
              fill="rgba(192,132,252,0.15)" stroke={PURPLE} strokeWidth={1.5}
            />
            {/* Arms */}
            ...
            />
            <path
              d={`M60 ${40 + weightY} L60 ${80 + weightY}`}
              stroke={GOLD}
              strokeWidth={1.5}
              strokeDasharray="4,3"
              opacity={0.6}
          ...
          <div
            style={{
              ...
              whiteSpace: "nowrap",
              textShadow: "0 0 14px rgba(240,191,106,0.5)",
            }}
          >
            重心を下に
          </div>
