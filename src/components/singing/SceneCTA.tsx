...
import { ShortsCTA } from "../ShortsCTA";
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 60, opacity }}>
      {bars.map((i) => {
        ...
        return (
          <div key={i} style={{ width: 6, height: h, borderRadius: 3, background: `rgba(192,132,252,${0.3 + Math.abs(Math.sin(frame * 0.12 + i * 0.55)) * 0.6})` }} />
        );
      })}
    </div>
  );
  ...
  return (
    <div style={{ opacity }}>
      <svg width={400} height={100} viewBox="0 0 400 100" overflow="visible">
        <path d={d} fill="none" stroke={GOLD} strokeWidth={2.5} strokeLinecap="round" style={{ filter: "drop-shadow(0 0 6px rgba(240,191,106,0.5))" }} />
        {progress >= 1 && <circle cx={380} cy={20} r={5} fill={GOLD} style={{ filter: "drop-shadow(0 0 8px #F0BF6A)" }} />}
      </svg>
    </div>
  );
};
...
export const SceneCTA: React.FC = () => {
  ...
  const animatedButton = (
    <div
      style={{
        ...
        borderRadius: 72,
        paddingTop: 34, paddingBottom: 34, paddingLeft: 72, paddingRight: 72,
        fontSize: 52, fontWeight: 700, color: WHITE, letterSpacing: "0.04em",
        fontFamily: FONT,
        boxShadow: `0 0 56px rgba(147,51,234,${glowAlpha}), 0 0 112px rgba(147,51,234,${glowAlpha * 0.4}), 0 8px 40px rgba(0,0,0,0.5)`,
  ...
  const noteContent = (
    <div style={{ opacity: noteOpacity, marginTop: 32, textAlign: "center" }}>
      <div style={{ fontSize: 38, fontWeight: 500, color: "rgba(255,255,255,0.88)", letterSpacing: "0.06em", fontFamily: FONT, textShadow: "0 2px 10px rgba(0,0,0,0.6)", marginBottom: 14 }}>
        プロフィール・説明欄のリンクから
      </div>
      <div style={{ fontSize: 30, fontWeight: 400, color: "rgba(192,132,252,0.85)", letterSpacing: "0.05em", fontFamily: MONO, textShadow: "0 1px 8px rgba(0,0,0,0.7)" }}>
        be-my-style.com
      </div>
      <div style={{ opacity: 0.6, marginTop: 10, fontSize: 24, fontWeight: 400, color: "rgba(192,132,252,0.65)", letterSpacing: "0.04em", fontFamily: MONO, textShadow: "0 1px 6px rgba(0,0,0,0.7)" }}>
    ...
    <ShortsLayout
      ...
      background={
        <>
          ...
          {/* Center premium glow */}
          <div style={{ position: "absolute", top: "38%", left: "50%", transform: "translate(-50%, -50%)", width: 900, height: 900, borderRadius: "50%", background: "radial-gradient(circle, rgba(123,47,190,0.18) 0%, transparent 60%)", pointerEvents: "none" }} />
        </>
      }
      brand="BeMyStyle"
      brandSubtitle="AI 歌唱診断"
      showBrand
      brandOpacity={interpolate(frame, [12, 28], [0, 0.9], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })}
      brandAlign="center"
      brandFontSize={36}
      brandSubtitleFontSize={26}
      contentStyle={{ padding: "0 64px" }}
    >
      {/* Wave visualizer */}
      ...
      {/*
       * ShortsCTA handles headline + subtitle + ctaButton layout.
       * Wave / curve animations are passed via inline animated wrappers.
       */}
      <div style={{ transform: `translateY(${headY}px)`, opacity: headOpacity, width: "100%" }}>
        <ShortsCTA
          title="AI歌唱診断で"
          subtitle={
            <span style={{ transform: `translateY(${subY}px)`, display: "inline-block", opacity: subOpacity }}>
              今の歌声をチェック
            </span>
          }
