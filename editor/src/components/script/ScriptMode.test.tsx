...
describe('ScriptMode: 「この台本で動画を作る」（Wizardへの受け渡し）', () => {
  const btn = () => screen.queryByRole('button', { name: /この台本で動画を作る|移動中/ }) as HTMLButtonElement | null

  it('onCreateVideo が無ければボタンを出さない（従来どおり）', async () => {
    render(<ScriptMode />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
    expect(btn()).toBeNull()
  })
  it('台本を選ぶまでは出ず、選ぶと押せる', async () => {
    render(<ScriptMode onCreateVideo={vi.fn()} />)
    expect(btn()).toBeNull()
    await generateOk()
    expect(btn()).toBeNull()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
  ...
  it('編集後の最新の台本を渡し、編集前の本文は渡さない。タイトルはコンセプトの1行目', async () => {
    const onCreate = vi.fn()
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk('歌が上手くなる方法\n初心者向け')
    fireEvent.click(screen.getByRole('radio', { name: /共感系/ }))
    fireEvent.change(screen.getByLabelText(/台本を編集/), { target: { value: '編集した台本です。' } })
    fireEvent.click(btn()!)
    expect(onCreate).toHaveBeenCalledTimes(1)
    expect(onCreate).toHaveBeenCalledWith({ title: '歌が上手くなる方法', script: '編集した台本です。' })
    expect(JSON.stringify(onCreate.mock.calls[0][0])).not.toContain('共感系の台本です。')
  })
  it('最新の分割結果があれば一緒に渡す', async () => {
    const onCreate = vi.fn()
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
    fireEvent.change(screen.getByLabelText(/台本を編集/), { target: { value: '編集済み' } })
    fetchMock.mockResolvedValueOnce(res(200, { ok: true, slides: [{ text: '冒頭' }, { text: '本題' }] }))
    fireEvent.click(screen.getByRole('button', { name: /スライド単位に分割/ }))
    await screen.findByRole('region', { name: '分割結果' })
    fireEvent.click(btn()!)
    expect(onCreate).toHaveBeenCalledWith({ title: '歌が上手くなる方法', script: '編集済み', slides: [{ text: '冒頭' }, { text: '本題' }] })
  })
  it('分割後に編集したら、古い分割結果は渡さない', async () => {
    const onCreate = vi.fn()
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
    fetchMock.mockResolvedValueOnce(res(200, { ok: true, slides: [{ text: '古い分割' }] }))
    fireEvent.click(screen.getByRole('button', { name: /スライド単位に分割/ }))
    await screen.findByRole('region', { name: '分割結果' })
    fireEvent.change(screen.getByLabelText(/台本を編集/), { target: { value: '分割後に編集した台本' } })
    fireEvent.click(btn()!)
    expect(onCreate).toHaveBeenCalledWith({ title: '歌が上手くなる方法', script: '分割後に編集した台本' })
    expect(JSON.stringify(onCreate.mock.calls[0][0])).not.toContain('古い分割')
  })
  it('空白だけの台本には押せず、渡さない', async () => {
    const onCreate = vi.fn()
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
    fireEvent.change(screen.getByLabelText(/台本を編集/), { target: { value: '  \n ' } })
    expect(btn()).toBeDisabled()
    fireEvent.click(btn()!)
    expect(onCreate).not.toHaveBeenCalled()
  })
  it('連打しても受け渡しは1回。ボタンを押しても外部APIは呼ばない', async () => {
    const onCreate = vi.fn()
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
  ...
  it('受け渡しに失敗（例外）したら押し直せる', async () => {
    const onCreate = vi.fn().mockImplementationOnce(() => { throw new Error('x') })
    render(<ScriptMode onCreateVideo={onCreate} />)
    await generateOk()
    fireEvent.click(screen.getByRole('radio', { name: /教育系/ }))
    fireEvent.click(btn()!)
