...
interface Status {
  config: { digest: { enabled: boolean; durationSec: number; grayscale: boolean; bgm: { volume: number; fadeInSec: number; fadeOutSec: number; credit: { title: string; composer: string } } }; lineIntro: { enabled: boolean; durationSec: number; showQr: boolean }; lineOutro: { enabled: boolean; durationSec: number; showQr: boolean }; preview: { digest: boolean; lineIntro: boolean; lineOutro: boolean } }
  assets: { bgm: AssetState; qr: AssetState }
...
export function CompositionSettingsPanel({ value, onChange, disabled }: { value: CompositionOverrides & { useComposition?: boolean }; onChange: (v: CompositionOverrides & { useComposition?: boolean }) => void; disabled?: boolean }) {
  ...
  const set = (patch: CompositionOverrides) => onChange({ ...value, digest: { ...value.digest, ...patch.digest, bgm: { ...value.digest?.bgm, ...patch.digest?.bgm } }, lineIntro: { ...value.lineIntro, ...patch.lineIntro }, lineOutro: { ...value.lineOutro, ...patch.lineOutro }, preview: { ...value.preview, ...patch.preview } })
