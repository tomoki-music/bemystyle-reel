...
export async function renderCompositionToFile(p) {
  ...
  try {
    ...
    if (p.cfg.mainBgm?.enabled) {
      ...
      const r = await prepareMainBgm({ cfg: p.cfg, roots: p.mainBgmRequest.roots, sourcePath: p.sourcePath, mainItems: p.mainItems ?? p.mainBgmRequest.mainItems, mainSec: mainSec.endSec - mainSec.startSec, tmpDir: bgmTmp }, { spawnFn: p.spawnFn })
      if (!r.ok) throw new Error(r.error)
      mainBgm = { inputPath: r.prep.inputPath, plan: r.prep.plan, gainDb: r.prep.gainDb }
      mainBgmReport = { gain: r.prep.gain, levels: r.prep.levels, info: r.prep.info, plan: r.prep.plan }
    }
